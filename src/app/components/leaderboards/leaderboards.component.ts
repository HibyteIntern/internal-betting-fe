...
export class LeaderboardsComponent {
  ...
  private triggerConfetti() {
    ...
    setTimeout(() => {
      if (this.firstPlacePodium && this.firstPlacePodium.nativeElement) {
        this.confettiService.runConfettiFromElement(
          this.firstPlacePodium.nativeElement,
        );
      }
    }, 800);
  }
  ...
  applyFilter(event: Event): void {
    ...
    if (!formattedFilterValue) {
      ...
      this.filteredDataSource = this.dataSource.filter((item) =>
        item.username?.toLowerCase().includes(formattedFilterValue),
      );
    }
  }
  ...
  fetchUserProfiles(userIds: number[]): void {
    const userProfilesRequests = userIds.map((userId) =>
      this.userProfileService.getById(userId).toPromise(),
    );

    Promise.all(userProfilesRequests)
      .then((userProfiles) => {
        const mappedProfiles = userProfiles.map((profile, index) => {
          const leaderboardEntry = this.leaderboard?.entries.find(
            (entry) => entry.userId === profile?.userId,
          );
          const metricValue = leaderboardEntry
            ? leaderboardEntry.metrics[this.selected]
            : 0;
          let positionClass = '';
          if (index === 0) {
        ...
        this.dataSource = mappedProfiles.filter(
