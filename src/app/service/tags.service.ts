...
export class TagsService {
  ...
  private selectedTags: string[] = ['Events', 'Competitions'];
  ...
  public addTag(tagIndex: number): void {
    if (
      this.availableTags[tagIndex] === 'Events' ||
      this.availableTags[tagIndex] === 'Competitions'
    ) {
  ...
  public removeTag(tagIndex: number): void {
    if (
      this.selectedTags[tagIndex] === 'Events' ||
      this.selectedTags[tagIndex] === 'Competitions'
    ) {
  ...
  public isEventsSelected() {
    return (
      this.selectedTags[0] === 'Events' || this.selectedTags[1] === 'Events'
    );
  }

  public isCompetitionsSelected() {
    return (
      this.selectedTags[0] === 'Competitions' ||
      this.selectedTags[1] === 'Competitions'
    );
  }
}
